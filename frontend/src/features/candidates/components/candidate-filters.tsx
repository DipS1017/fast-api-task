...
type TextState = Pick<FiltersState, "keyword" | "role_applied" | "skill">;
...
export function CandidateFilters({ filters, onChange }: CandidateFiltersProps) {
  ...
  const pushText = useDebouncedCallback((nextText: TextState) => {
    onChange({ ...filters, ...nextText });
  }, DEBOUNCE_MS);

  function onText(key: keyof TextState, value: string) {
